...
spawnPlayer()
{
	...
	if(self.pers["team"] == "allies")
		self setClientCvar("cg_objectiveText", &"Hunt the props!");
	else if(self.pers["team"] == "axis")
		self setClientCvar("cg_objectiveText", &"Don not get killed by seekers!");
	...
	self thread prophunt_init();

	if(self.pers["team"] == "axis" || self.sessionteam == "axis")
	{
		iprintln("Work?");

	    self thread xmodel_change();
	}

	self thread checkdeath();
	if(getEntArray("player", "classname").size >= 2)
	{
		level.randPlr = getRandomPlayer();
		level.randPlr.sessionteam = "allies";
		level.randPlr iPrintLnBold("You've been choosen to hunt the props");
		level.randPlr giveWeapon(self.pers["weapon"]);
		level.randPlr giveMaxAmmo(self.pers["weapon"]);
		level.randPlr setClientCvar("cg_thirdPerson", 0);
	}
}

model_precache()
{
    precacheModel("xmodel/barrels");
    precacheModel("xmodel/VaseFlowers");
    precacheModel("xmodel/tree_winter_pine");
    precacheModel("xmodel/Tub");
    precacheModel("xmodel/tree_towntree1");
    precacheModel("xmodel/Spruce");
    precacheModel("xmodel/Toilet");
    precacheModel("xmodel/tombstone1");
    precacheModel("xmodel/Sink");
    precacheModel("xmodel/rock_big");
    precacheModel("xmodel/Pine");
    precacheModel("xmodel/lamp_desk_on");
    precacheModel("xmodel/FullBed");
    precacheModel("xmodel/crate_misc2");
...
xmodel_change()
{
    ...
    self.modelList[0] = "xmodel/barrels";
    self.modelList[1] = "xmodel/VaseFlowers";
    self.modelList[2] = "xmodel/tree_winter_pine";
    self.modelList[3] = "xmodel/Tub";
    self.modelList[4] = "xmodel/tree_towntree1";
    self.modelList[5] = "xmodel/Spruce";
    self.modelList[6] = "xmodel/Toilet";
    self.modelList[7] = "xmodel/tombstone1";
    self.modelList[8] = "xmodel/Sink";
    self.modelList[9] = "xmodel/rock_big";
    self.modelList[10] = "xmodel/Pine";
    self.modelList[11] = "xmodel/lamp_desk_on";
    self.modelList[12] = "xmodel/gib_stone1";
    self.modelList[13] = "xmodel/FullBed";
    self.modelList[14] = "xmodel/crate_misc2";
    self.modelList[15] = "xmodel/cow_dead";

    self.currentModelIndex = 0;
	//self detachAll();
	self takeAllWeapons();
    //self setModel(self.modelList[self.currentModelIndex]); // Start with first model
    ...
    {
		if(self.pers["team"] == "allies")
			return; // Allies = seekers, can't change model
        if (self meleebuttonpressed())
        {
            self detachAll();

            self.currentModelIndex++;
            if (self.currentModelIndex >= self.modelList.size)
            {
                self.currentModelIndex = 0;
            }

            self setModel(self.modelList[self.currentModelIndex]);
			self solid();
		...
		if(self.pers["team"] == "allies")
            ...
            {
                self.currentModelIndex = self.modelList.size - 1;
            }
...
getRandomPlayer()
{
    ...
    for (i = 0; i < players.size; i++)
    {
        if (isDefined(players[i]) && players[i].sessionstate == "playing" && isAlive( players[i]))
        {
    ...
    {
        index = randomInt(alivePlayers.size);
        return alivePlayers[index];
    }

    return undefined; // No valid player
}
